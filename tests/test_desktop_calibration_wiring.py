...
def test_start_edge_dense_calibration_uses_experimental_geometry(
    qt_app: QApplication,
) -> None:
    from desktop_demo.ui.main_window import MainWindow
    ...
    flow = window.calibration_view.flow
    assert len(flow.targets) == 17
    assert [target.id for target in flow.targets[0:7]] == [
        "top0",
        "top1",
        "top2",
        "top3",
    ...
    assert isinstance(window.calibration_session.model, LinearRidgeCalibrationModel)
    assert window.calibration_session.model.alpha == 1.0
    assert window.calibration_view.title_label.text() == "17-point edge-dense calibration"
    assert window.calibration_target_overlay.isVisible()
    window.close()
    qt_app.processEvents()


def test_start_top_left_focus_calibration_uses_experimental_geometry(
    qt_app: QApplication,
) -> None:
    from desktop_demo.ui.main_window import MainWindow
    ...
    window.start_top_left_focus_calibration()

    flow = window.calibration_view.flow
    assert len(flow.targets) == 25
    assert [target.id for target in flow.targets[0:14]] == [
        "top0",
        "top1",
        "top2",
        "top3",
    ...
    assert isinstance(window.calibration_session.model, LinearRidgeCalibrationModel)
    assert window.calibration_session.model.alpha == 1.0
    assert window.calibration_view.title_label.text() == "25-point top-left focus calibration"
    assert window.calibration_target_overlay.isVisible()
    window.close()
    qt_app.processEvents()

...
- [ ] When comparing repeated live runs, run `uv run python tools/analyze_repeat_run_diagnostics.py metrics/demo.jsonl --run START1:END1 --run START2:END2 --screen-width WIDTH --screen-height HEIGHT --grid-columns 4 --grid-rows 3` and confirm it reports per-target signed residuals, grid collapse/recovery flags, and predicted grid-cell distributions from scalar telemetry only.

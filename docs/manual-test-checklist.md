...
- [ ] Run `uv run python tools/evaluate_calibration_models.py metrics/demo.jsonl --screen-width 1512 --screen-height 982 --grid-columns 4 --grid-rows 3 --objective grid --calibration-sample-window middle` with the screen dimensions from the manual run and confirm it ranks candidate calibration models. Use `--calibration-sample-window all|early|middle|late` to compare capture timing. The evaluator includes replay-only target-weighted candidates for vertical edges, screen edges, and corners, replay-only asymmetric quadrant correction candidates, replay-only vertical-bias and per-band correction candidates, plus replay-only pose-normalized candidates when the log contains the 23-value head-pose feature vector. Add `--include-target-residuals` to append per-target calibration/validation residual tables for the top-ranked model, including validation predicted-cell distributions for grid-collapse regressions.
- [ ] When comparing repeated live runs, run `uv run python tools/analyze_repeat_run_diagnostics.py metrics/demo.jsonl --run START1:END1 --run START2:END2 --screen-width WIDTH --screen-height HEIGHT --grid-columns 4 --grid-rows 3` and confirm it reports per-target signed residuals, grid collapse/recovery flags, predicted grid-cell distributions, and named calibration feature-drift deltas from scalar telemetry only.
